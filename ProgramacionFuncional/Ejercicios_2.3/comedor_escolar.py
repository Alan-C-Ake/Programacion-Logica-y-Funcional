...
#  Hecho por Alan Alberto Colli Ake 8-B
# =============================================================================
...
#    R: Es una función que puede ser tratada como cualquier otra variable se puede asignar a una variable, 
#       pasar como argumento a otras funciones o ser retornada por otra función.
#
# 2. ¿Cuál es la diferencia entre una función de orden superior y un callback?
#    R: Una función de orden superior es la que recibe otra función como argumento o la retorna. 
#       El callback es precisamente esa función que es pasada como argumento para ser ejecutada después.
#
# 3. ¿Cuándo conviene usar comprensión de listas en lugar de un ciclo for?
#    R: Cuando necesitamos crear una nueva lista transformando o filtrando elementos de una secuencia existente,
#       es más concisa y legible.
#
# 4. ¿Qué hace map() y cómo se relaciona con lambda?
#    R: map() aplica una función a cada elemento de un iterable (como una lista). Se relaciona con lambda porque 
#       frecuentemente usamos una función anónima (lambda) dentro de map() para definir operaciones rápidas en una sola 
#       línea sin necesidad de definir una función formal con 'def'.
#
# 5. ¿Qué ventaja ofrece pasar una función como argumento a otra función?
#    R: Permite abstraer y reutilizar el código. Separa la lógica de control (el "cuándo" o "cómo" iterar) de la lógica 
#       de negocio (el "qué" hacer), haciendo el sistema más modular y flexible.
# ─────────────────────────────────────────────────────────────────────────────
...
#        R: preparar_pizza, preparar_agua, preparar_tamal, calcular_promocion y tomar_orden.
#      - ¿Cuál de ellas es de orden superior? ¿Por qué?
#        R: tomar_orden, porque recibe por parámetro la función del producto a preparar (preparar_alimento).
#      - ¿Dónde usarás comprensión de listas?
#        R: Al generar la lista de porciones solicitadas iterando sobre la cantidad pedida.
#      - ¿Dónde usarás lambda + map()?
#        R: Al calcular la lista de precios, asignando el precio unitario a cada porción generada.
# ─────────────────────────────────────────────────────────────────────────────
...
def tomar_orden(preparar_alimento, cantidad, precio_unitario):
    # a) Comprensión de listas: Llama al callback la cantidad de veces solicitada
    porciones = [preparar_alimento() for _ in range(cantidad)]
    
    # b) map() + lambda para precios: Mapea cada elemento de 'porciones' a su precio
...
total_pizza = sum(map(lambda precio: precio, precios_pizza))
total_agua = sum(map(lambda precio: precio, precios_agua))
total_tamal = sum(map(lambda precio: precio, precios_tamal))
total_global = total_pizza + total_agua + total_tamal

print("\n========== TOTALES ==========")
print(f"Total Pizzas: ${total_pizza}")
print(f"Total Aguas:  ${total_agua}")
print(f"Total Tamales:${total_tamal}")
print(f"TOTAL A PAGAR:${total_global}")
print("=============================\n")
...
def elegir_producto(nombre):
    ...
    if nombre == "pizza":
        ...
        return lambda: "producto desconocido"
# 3. Usa la función del punto 2 para reemplazar los argumentos directos en
#    las llamadas a tomar_orden().
orden_pizza_extra  = tomar_orden(elegir_producto("pizza"),  cantidad_pizzas,  25)
orden_agua_extra   = tomar_orden(elegir_producto("agua"),   cantidad_aguas,   10)
orden_tamal_extra  = tomar_orden(elegir_producto("tamal"),  cantidad_tamales, 15)
# ─────────────────────────────────────────────────────────────────────────────